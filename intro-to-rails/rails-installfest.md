...
- [Rails template](https://guides.rubyonrails.org/rails_application_templates.html), which is one method to set-up how all new Rails applications should look on a local machine. In [a future lesson](rails-template-setup.md), we will set this up once, so that we may scaffold every Rails project with our preferred configurations at Ada.
